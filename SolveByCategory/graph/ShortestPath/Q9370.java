...
// ArrayList<Node>[] SolveByCategory.graph = new ArrayList[V];
// PriorityQueue<Integer> target_list = new PriorityQueue<>();
// for (int i = 0; i < V; i++)
// SolveByCategory.graph[i] = new ArrayList<Node>();
...
// SolveByCategory.graph[u].add(new Node(v, weight));
// SolveByCategory.graph[v].add(new Node(u, weight));
// }
...
// dist[i] = dijkstra(SolveByCategory.graph, starting_point[i] - 1, V);
// }
...
// public static int[] dijkstra(ArrayList<Node>[] SolveByCategory.graph, int start, int V) {
// int[] dist = new int[V];
...
// if (!visit[now.to]) {
// visit[now.to] = true;
// for (Node next : SolveByCategory.graph[now.to]) {
