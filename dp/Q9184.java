 ...
 * 
 * public class Main { static int dp[][][] = new int[21][21][21];
 * 
 ...
 * dp[0][i][j] = 1; dp[i][0][j] = 1; dp[i][j][0] = 1; } } while (true) {
 ...
 * (int k = 1; k < c + 1; k++) { if (dp[i][j][k] != 0) continue; if (i < j && j
 * < k) { dp[i][j][k] = dp[i][j][k - 1] + dp[i][j - 1][k - 1] - dp[i][j - 1][k];
 * } else { dp[i][j][k] = dp[i - 1][j][k] + dp[i - 1][j - 1][k] + dp[i - 1][j][k
 * - 1] - dp[i - 1][j - 1][k - 1]; } } } } return dp[a][b][c]; }
 * 
 * }
 */
